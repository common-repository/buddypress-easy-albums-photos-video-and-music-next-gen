			...
			<div id="item-body">
				...
				?>
						<div style='float:right;margin-top:10px;'>
							<a href='<?php echo wp_nonce_url(bp_displayed_user_domain() . bp_current_component() . "?action=create&tab={$current_tab}", 'bp_easyalbum_screen_create')?>' id="createBT" class='albumsBT'><?php _e("+ Create Album",'bp-easyalbums')?></a>
						</div>
				<?php 
					}
				?>
				...
				$sql = "SELECT * FROM {$bp->easyalbums->table_name} WHERE uID='$_uid' AND gal_type<=$perms AND status='1' AND tab='{$current_tab}' ORDER BY ID";
								...
								<div class='easyalbums_thumbslider'>
									...
									?>
											<div class="easyalbums_gallerycontrol_inner" >
												<a href='<?php printf( '%s', wp_nonce_url( bp_displayed_user_domain() . bp_current_component() ."?action=view&fid=".$gal->fID, 'bp_easyalbums_screen_publish' ) ) ?>' ><?php _e('View Album','bp-easyalbums')?></a>
												<a href='<?php printf( '%s', wp_nonce_url( bp_displayed_user_domain() . bp_current_component() . '?action=edit&fid='.$gal->fID, 'bp_easyalbums_screen_edit' ) ) ?>'><?php _e('Album Manager','bp-easyalbums')?></a>
												<a href='<?php printf( '%s', wp_nonce_url( bp_displayed_user_domain() . bp_current_component() ."?action=publish&fid=".$gal->fID, 'bp_easyalbums_screen_publish' ) ) ?>' onclick='return (confirm("The album will be published to the activity stream and will be available for view to all! are you sure?"))'><?php _e('Publish to activity','bp-easyalbums')?></a>
												<a href='javascript:void(0)' onclick='var _a = confirm("<?php _e('Are you sure you want to delete this album?','bp-easyalbums')?>");if(_a)window.location="<?php printf( '%s', wp_nonce_url( bp_displayed_user_domain() . bp_current_component() . '?action=delete&fid='.$gal->fID, 'bp_easyalbums_screen_delete' ) ) ?>";'><?php _e('Delete Album','bp-easyalbums')?></a> 
											</div>
									<?php 
										}
									?>
